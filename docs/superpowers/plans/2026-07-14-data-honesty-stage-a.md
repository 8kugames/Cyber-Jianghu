...
### Task 5: item_type 枚举统一（修复 Material/Tool 降级 + 删除 armor 死代码）

**变体集合决策（代码事实决定）：** 5 变体 Consumable/Weapon/Currency/Material/Tool——全部由 items.yaml 配置支撑。**不含 Armor**（items.yaml 零 armor 物品；agent `engine_prompts.rs:371` 认 armor 只是 prompt 兜底过度泛化；manager.rs:347 的 "armor" 是永不命中死代码）。以 protocol/sqlx_types.rs 为权威定义。
...
- Modify: `crates/agent/src/soul/actor/engine_prompts.rs:365-373`

- [ ] **Step 1: 扩展 protocol ItemType 到 5 变体**
...
```
更新 Display impl（sqlx_types.rs:125-133）和 FromStr impl（sqlx_types.rs:135-146）补上 Material/Tool 分支。
...
- [ ] **Step 4: 删除 manager.rs armor 死代码**

`crates/server/src/inventory/manager.rs:347`，把：
```rust
if matches!(config.item_type.as_str(), "weapon" | "armor") {
```
改为（config.item_type 现在是 ItemType 枚举；删除永不命中的 armor 匹配）：
```rust
if matches!(config.item_type, ItemType::Weapon) {
```

- [ ] **Step 5: 删除 agent engine_prompts.rs armor 兜底分支**

`crates/agent/src/soul/actor/engine_prompts.rs:365-373`，删除 match 里的 `"armor" => ...` 分支（items.yaml 无 armor 物品，该分支永不触发）。
...
```bash
git add -A
git commit -m "fix(protocol): ItemType 统一为 5 变体——修复 Material/Tool 降级 + 删除 armor 死代码"
```
...
```rust
/// 动作效果类型（3 值闭集，均由 actions.yaml 配置支撑）
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum EffectType {
    AttributeChange,
...
> **删除 remove_item/teleport**：这两个是 `#[allow(dead_code)]` 预留常量，actions.yaml 零配置、执行器零实现、全仓零调用。臆造的死能力，删除而非枚举化。绝不用 `warn!` 给永不触发的分支盖实现戳。
...
- [ ] **Step 3: 改 server ActionEffect.effect_type + 删臆造常量**

`crates/server/src/game_data/types/actions.rs:315`：`pub effect_type: String` → `pub effect_type: EffectType`。删除整个 `#[allow(dead_code)] impl ActionEffect` 块（lines 387-394，含 EFFECT_TYPE_ATTRIBUTE_CHANGE/ATTRIBUTE_MAX_CHANGE/ADD_ITEM/REMOVE_ITEM/TELEPORT 全部常量——枚举已取代它们）。

- [ ] **Step 4: 改 executor/mod.rs dispatch——穷尽匹配**

`crates/server/src/actions/executor/mod.rs:135-174`，改为穷尽匹配（3 变体，无 `_=>{}` 兜底）：
```rust
...
```bash
git add -A
git commit -m "refactor(protocol): EffectType 枚举化 3 变体——删除 remove_item/teleport 臆造常量"
```
...
> **删除 location/skill**：`#[allow(dead_code)]` 预留常量，actions.yaml 零配置、执行器/验证器零实现、全仓零调用。臆造的死需求，删除。

- [ ] **Step 2: 改 ActionRequirementInfo + ActionRequirement 字段 + 删臆造常量**

protocol entities.rs:304 和 server actions.rs:274：`String` → `RequirementType`。删除整个 `#[allow(dead_code)] impl ActionRequirement` 块（actions.rs:337-342）。

- [ ] **Step 3: 改 executor/mod.rs + validator.rs dispatch——穷尽匹配**

两处 match 改穷尽匹配（2 变体，无 `_=>{}` 兜底）：
```rust
match &req.requirement_type {
    RequirementType::Attribute => { /* 现有逻辑 */ }
    RequirementType::Item => { /* 现有逻辑 */ }
}
```
...
```bash
git add -A
git commit -m "refactor(protocol): RequirementType 枚举化 2 变体——删除 location/skill 臆造常量"
```
...
改 `processor.rs` 的 `process_single_intent` 签名，接收 `&mut tx` 或返回需持久化的 state 让 realtime 在同一 tx 内 upsert。改 `realtime.rs:247-263` 传 tx 而非 pool。upsert 签名从 `&PgPool` 改为 `&mut PgConnection`（参照 `batch_insert_action_logs` 的 `state_ops.rs:415-417` 范本）。

**CAS 失败语义（必须显式处理）**：
- upsert 用乐观锁 `WHERE state_version=$6`（state_ops.rs:289）。CAS 失败 → upsert 返 Err → tx 全回滚（inventory/ground_items/action_log 一并回滚）。
- **这修复了当前的不一致**：当前 upsert 在 tx 外，CAS 失败时"物品已落库/状态未落库"；纳入后变成"全部一致回滚"。
- CAS 失败后：intent 视为失败，发 persist_failed 给 agent，**不 retry**。
- **单消费者 FIFO 下 CAS 几乎不可能失败**。唯一竞态：TickBoundary 把 `state_version` reset 为 0（realtime.rs:577）与 in-flight intent 持有旧 version 的交错。需确认 TickBoundary 与 Intent 在同一 channel 串行处理（当前架构 `realtime.rs:116` 确实如此），reset 后不会有旧 version 的 intent 进入 tx。若发现此场景，需在 tick 边界期间暂停 intent 入队。
