...
async function openAgentModal(agentId) {
  ...
  try {
    ...
    // 展示名映射 + 天魂层名映射：必须在渲染前就绪
    await Promise.allSettled([loadDisplayMap(), getLayerDisplay()]);
    ...
    document.getElementById("modal-tab-experiences").innerHTML =
      ...
      '<p style="margin-bottom:12px;">经历日志已迁移至独立页面</p>' +
      '<a href="history.html?agent=' + escapeHtml(agentId) + '" target="_blank" style="color:var(--accent,#14b8a6);font-size:14px;">查看全部经历日志 →</a>' +
      '</div>';
