...
```
Phase 0: 加载状态 --> Phase 1: 意图收集 --> 验证 --> 冲突解析 --> 执行 --> Phase 2: 衰减处理 --> Phase 3: 统计 --> Phase 4: 持久化 --> Phase 5: 广播
```
...
| 阶段 | 说明 |
|------|------|
| Phase 0 | 从 PostgreSQL 加载所有 Agent 状态 |
| 意图收集 | 收集所有存活 Agent 的 Intent |
| 验证 | 检查动作合法性 |
| 冲突解析 | 处理多意图冲突 |
| 执行 | 按优先级执行动作 |
| Phase 2: 衰减处理 | 属性衰减、物品耐久等（在意图执行之后） |
| Phase 3: 统计 | 汇总本 Tick 统计数据 |
