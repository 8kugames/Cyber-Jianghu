...
use state::{create_rate_limiter, start_rate_limiter_cleanup, AppState};
...
fn start_tick_engine(
    ...
    tokio::spawn(async move {
        let mut tick_scheduler =
            TickScheduler::new(config, db_pool, connection_manager, intent_manager);
...
async fn main() -> Result<()> {
    ...
    info!("Tick周期: {}秒", config.tick_engine.tick_duration_secs);
    info!("服务启动完成，等待连接...");
    info!(
