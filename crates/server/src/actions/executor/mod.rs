...
use crate::game_data::{
    ActionEffect, ActionRegistry, ActionRequirement,
};
...
impl ActionExecutor {
    ...
    pub fn execute(&self, intent: &Intent, agent_state: &mut AgentState) -> ActionExecutionResult {
        ...
        let mut result = match intent.action_type.as_str() {
            ...
            "speak" => {
                BasicActionExecutor::execute_speak(intent, intent.action_data.clone())
            }
            ...
            "pickup" => {
                BasicActionExecutor::execute_pickup(
                    intent,
                    intent.action_data.clone(),
                    &agent_state.node_id.clone(),
                )
            ...
            "drop" => {
                BasicActionExecutor::execute_drop(
                    intent,
                    intent.action_data.clone(),
                    &agent_state.node_id.clone(),
                )
            ...
            "gather" => {
                BasicActionExecutor::execute_gather(
                    intent,
                    intent.action_data.clone(),
                    &agent_state.node_id.clone(),
                )
            ...
            "craft" => {
                BasicActionExecutor::execute_craft(
                    intent,
                    intent.action_data.clone(),
                )
            }
            ...
            "trade" => {
                InteractionActionExecutor::execute_trade(intent, intent.action_data.clone())
            }
            _ => {
            ...
            for req in &config.requirements {
                ...
                match req.requirement_type.as_str() {
                    ActionRequirement::REQUIREMENT_TYPE_ATTRIBUTE => {
                        ...
                        let consume = req.get_bool("consume", false);
                        ...
                        if consume {
                            ...
                            if agent_state
                                .status
                                .apply_change(attribute, cost_value, &context)
                                .is_err()
                            {
                                return Err(format!("无法消耗属性 {}", attribute));
                            }
                        }
                    }
