...
use super::types::ProposalEvidence;

/// 从提案 IR 生成 actions.yaml 条目
///
/// 返回 `(action_name, entry_value)`，caller 负责写入文件。
pub fn generate_action_config(evidence: &ProposalEvidence) -> Result<(String, serde_yaml::Value)> {
    let action_name = evidence.proposed_action_type.clone();
    let category = infer_category(&evidence.ir.effect_refs);
    ...
    let requirements = build_requirements(&evidence.ir.requirement_refs);
    ...
    info!(
        ...
        category = %category,
        effect_refs = ?evidence.ir.effect_refs,
        requirement_refs = ?evidence.ir.requirement_refs,
...
mod tests {
    use super::*;
    use crate::governance::types::ProposalEvidence;
    use cyber_jianghu_protocol::types::governance::ProposedActionIR;
    use std::collections::HashMap;

    fn make_evidence(effect_refs: Vec<String>, requirement_refs: Vec<String>) -> ProposalEvidence {
        ProposalEvidence {
    ...
    fn test_infer_category_combat() {
        let evidence = make_evidence(vec!["combat.slash".into()], vec![]);
        let (name, entry) = generate_action_config(&evidence).unwrap();
    ...
    fn test_infer_category_social() {
        let evidence = make_evidence(vec!["dialogue.negotiate".into()], vec![]);
        let (_, entry) = generate_action_config(&evidence).unwrap();
        assert_eq!(entry["category"], "social");
    }

    #[test]
    fn test_infer_category_economic() {
        let evidence = make_evidence(vec!["craft.forge".into()], vec![]);
        let (_, entry) = generate_action_config(&evidence).unwrap();
    ...
    fn test_infer_category_default() {
        let evidence = make_evidence(vec!["misc.something".into()], vec![]);
        let (_, entry) = generate_action_config(&evidence).unwrap();
    ...
    fn test_build_requirements_with_tool_ref() {
        let evidence = make_evidence(vec![], vec!["tool.sword".into()]);
        let (_, entry) = generate_action_config(&evidence).unwrap();
    ...
    fn test_empty_rationale_uses_fallback() {
        let mut evidence = make_evidence(vec![], vec![]);
