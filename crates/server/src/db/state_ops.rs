...
pub async fn batch_insert_agent_states(pool: &PgPool, states: &[AgentState]) -> Result<()> {
    ...
    let serialized: Vec<(uuid::Uuid, i64, serde_json::Value, String, bool)> = states
        ...
        .map(|state| {
            ...
            Ok((state.agent_id, state.tick_id, attributes_json, state.node_id.clone(), state.is_alive))
        })
    ...
    query_builder.push_values(serialized, |mut b, (agent_id, tick_id, attributes_json, node_id, is_alive)| {
