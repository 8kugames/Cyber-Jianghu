...
use super::types::{build_game_rules_from_config, load_world_building_rules, WebSocketQuery};
...
async fn handle_websocket(
    ...
    // 发送注册成功确认（包含游戏规则）
    {
        ...
        let game_rules = build_game_rules_from_config(
            state.config.tick_engine.tick_duration_secs,
            state.game_data.get().game_rules.version.clone(),
        );
...
async fn handle_intent(
    ...
    if tick_diff < -TICK_WINDOW_SIZE || tick_diff > TICK_WINDOW_SIZE {
        warn!(
