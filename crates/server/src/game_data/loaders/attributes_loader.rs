...
pub fn load_unified_attributes<P: AsRef<Path>>(config_dir: P) -> Result<UnifiedAttributesConfig> {
    let file_path = config_dir.as_ref().join("attributes.json");

    let content = std::fs::read_to_string(&file_path)
        .with_context(|| format!("无法读取配置文件: {}", file_path.display()))?;

    let config: UnifiedAttributesConfig = serde_json::from_str(&content)
...
mod tests {
    ...
    fn test_load_unified_attributes() {
        ...
        assert!(attrs.data.derived.attributes.contains_key("max_carry_weight"));
    }
