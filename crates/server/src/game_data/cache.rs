    ...
    pub fn from_config(config: &super::types::UnifiedLocationsConfig) -> Self {
        ...
        for node in &config.data.nodes {
            let location_node = LocationNode {
                ...
                parent_id: if node.parent_id.is_empty() { None } else { Some(node.parent_id.clone()) },
