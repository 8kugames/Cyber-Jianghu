...
use crate::game_data::types::primary_attributes::{PRIMARY_ATTR_DAILY_RANDOM, PRIMARY_ATTR_GROWABLE, PRIMARY_ATTR_STATIC, PrimaryAttributesConfig};
...
impl AttributeComponent {
    ...
    pub fn from_config(config: &PrimaryAttributesConfig) -> Self {
        ...
        for (_attr_name, attr_def) in &config.attributes {
            // 使用字符串比较而非枚举模式匹配
    ...
    pub fn refresh_daily(&mut self) {
        ...
        for attr in self.collection.attributes.values_mut() {
            if matches!(attr.value, AttributeValue::DailyRandom { .. }) {
                if let Some((min, max)) = attr.metadata.birth_range {
    ...
    pub fn from_unified_config(config: &UnifiedAttributesConfig) -> Self {
        ...
        for (attr_name, attr_def) in &config.data.primary.attributes {
            ...
            let value = match metadata.attr_type {
                AttributeType::Growable => AttributeValue::Growable {
                    base: metadata.initial_value.unwrap_or(10),
                    current: metadata.initial_value.unwrap_or(10),
                },
