...
## 1. 设计目标
允许 Agent 一次性提交一个**原子意图队列**（Sequence of ATOMIC Intents），用于多个独立原子动作的排队执行（如"走到地点 A -> 拾取物品 B"），减少大模型在短时间内的频繁调用，节省算力开销。

**特别澄清**：
`subsequent_intents` 是一个**原子意图的执行队列**，**绝对不是复合动作（Composite Actions）**。每一个进入队列的 Intent 都必须是独立的、不可再分的原子行为。系统不支持也不允许提审复合行为，所有排队动作在服务端都会被逐个当做独立原子意图进行全套合法性校验和独立状态结算。
