...
use cyber_jianghu_agent::{
    dialogue::{DialogueClient, DialogueEventHandler},
...
use futures_util::future::BoxFuture;
    ...
    // 创建决策回调（简单的空闲决策）
    let decision_callback1 = Arc::new(|_: &cyber_jianghu_agent::WorldState| -> BoxFuture<'static, cyber_jianghu_agent::Intent> {
        Box::pin(async {
            cyber_jianghu_agent::Intent::idle(Uuid::new_v4(), 0)
        })
    });
    let decision_callback2 = Arc::new(|_: &cyber_jianghu_agent::WorldState| -> BoxFuture<'static, cyber_jianghu_agent::Intent> {
        Box::pin(async {
            cyber_jianghu_agent::Intent::idle(Uuid::new_v4(), 0)
        })
    });
    ...
    info!("   let message = dialogue_client1.request_dialogue(agent2_id, \"你好，能聊聊吗？\".to_string());");
    ...
    info!("   let reply = dialogue_client2.send_message(session_id, \"还不错，多谢关心。\".to_string());");
