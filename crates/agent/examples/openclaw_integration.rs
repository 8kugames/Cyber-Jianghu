    ...
    fn start() -> Self {
        ...
        tokio::spawn(async move {
            ...
            while let Some((prompt, reply_tx)) = rx.recv().await {
                ...
                println!("[OpenClaw] Received cognitive task (len: {}). Allocating GPU...", prompt.len());
...
impl LlmClient for OpenClawBridgeClient {
    async fn complete(&self, prompt: &str) -> Result<String> {
        ...
        self.system.task_queue.send((prompt.to_string(), tx)).await?;
...
async fn main() -> Result<()> {
    ...
    use cyber_jianghu_agent::{CognitiveEngineConfig, DynamicPersona};
