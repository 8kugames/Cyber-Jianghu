...
impl super::Agent {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        // 等待注册确认（包含游戏规则）
        let (_agent_id, game_rules) = self.client.wait_for_registration().await?;
        ...
        loop {
            ...
            if let Some(death_event) = world_state.events_log.iter().find(|e| {
                ...
                warn!("Agent '{}' has died: {}", self.config.agent.name, death_event.description);
            ...
            if world_state.tick_id % 84 == 0 {
                if let Err(e) = self.run_forgetting(world_state.tick_id).await {
