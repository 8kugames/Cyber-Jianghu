...
use cyber_jianghu_agent::config::{AgentConfig, MemoryConfig, PersonaConfig, ServerConfig};
use cyber_jianghu_agent::{
    ...
    runtime::decision::{http_decision, create_http_state, run_http_server},
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Run { mode, port, llm_provider, api_key, base_url, model }) => {
            run_agent(&mode, port, RunOptions {
                llm_provider,
                api_key,
                base_url,
                model,
...
async fn run_agent(mode: &str, port: u16, options: RunOptions) -> Result<()> {
    ...
    // 生成临时 agent_id（实际由服务器分配）
    let agent_id = Uuid::new_v4();

    // 如果是 http 模式，启动 HTTP Server（使用 runtime/decision/http.rs 的实现）
    let http_decision_state = if mode == "http" {
        // 如果 port 为 0，在 23340~23349 范围内随机选择端口
        let actual_port = if port == 0 {
            use rand::Rng;
            let random_port = rand::thread_rng().gen_range(23340..=23349);
        ...
        let (decision_state, api_state) = create_http_state(agent_id);
    ...
    let decision: Arc<dyn Fn(&WorldState) -> BoxFuture<'static, Intent> + Send + Sync> = match mode
    {
        "http" => {
            ...
            Arc::new(http_decision(agent_id, state, 55))
        }
        "cognitive" => {
            ...
            // 解析 LLM Provider
            let provider = LlmProvider::from_str(&options.llm_provider)
                .context(format!("Unknown LLM provider: {}. Valid options: openclaw, openai_compatible, ollama", options.llm_provider))?;

            // 获取 API Key（如果 provider 需要）
            let api_key = if provider.requires_api_key() {
                let key = options.api_key
                    .or_else(|| {
                        match provider {
            ...
            let dynamic_persona = DynamicPersona::new(
                agent_id,
                &config.agent.name,
        ...
        _ => {
            ...
            Arc::new(move |world_state: &WorldState| {
                ...
                Box::pin(async move {
                    error!("Unknown mode: {}. Supported modes: cognitive, http. Defaulting to idle.", mode);
