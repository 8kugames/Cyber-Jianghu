...
mod tests {
    ...
    fn create_test_memory(tick_id: i64, content: &str, importance: f32) -> MemoryEntry {
        MemoryEntry::new(Uuid::nil(), tick_id, content.to_string())
    ...
    async fn test_fifo_eviction() {
        ...
        backend.add(create_test_memory(1, "Event A", 0.5)).await.unwrap();
        backend.add(create_test_memory(2, "Event B", 0.5)).await.unwrap();
        backend.add(create_test_memory(3, "Event C", 0.5)).await.unwrap();
        assert_eq!(backend.count().await.unwrap(), 3);

        // 添加第 4 个事件，应该淘汰 Event A
        backend.add(create_test_memory(4, "Event D", 0.5)).await.unwrap();
        assert_eq!(backend.count().await.unwrap(), 3);
    ...
    async fn test_get_top_by_importance() {
        ...
        backend.add(create_test_memory(1, "Low", 0.2)).await.unwrap();
        backend.add(create_test_memory(2, "High", 0.9)).await.unwrap();
        backend.add(create_test_memory(3, "Medium", 0.5)).await.unwrap();
    ...
    async fn test_get_by_tick_range() {
        ...
        backend.add(create_test_memory(1, "Event 1", 0.5)).await.unwrap();
        backend.add(create_test_memory(5, "Event 5", 0.5)).await.unwrap();
        backend.add(create_test_memory(10, "Event 10", 0.5)).await.unwrap();
    ...
    fn test_build_context() {
        let mut backend = WorkingMemoryBackend::new(10);
        backend.events.push_front(create_test_memory(1, "你吃了馒头", 0.3));
        backend.events.push_front(create_test_memory(2, "你喝了水", 0.3));
