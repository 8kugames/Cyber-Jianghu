                ...
                if filename.starts_with("agent_") && filename.ends_with(".db") {
                        ...
                        .and_then(|s| s.strip_suffix(".db"))
                    {
                        if let Ok(agent_id) = Uuid::parse_str(agent_id_str) {
                            if let Some(lifetime) = self.inspect_agent_db(&path, agent_id).ok() {
    ...
    pub fn get_memories_in_range(
        ...
        for agent in &self.agents {
            ...
            if let (Some(earliest), Some(latest)) = (agent.earliest_tick, agent.latest_tick) {
                if latest < tick_start || earliest > tick_end {
...
mod tests {
    ...
    fn test_registry_with_agents() {
...

        {
            ...
            // 添加一些记忆
            let memory1 =
                crate::memory::store::ClientMemory::new(agent1_id, 1, "Agent1 记忆".to_string())
                    .with_importance(0.8);
            let memory2 =
                crate::memory::store::ClientMemory::new(agent2_id, 2, "Agent2 记忆".to_string())
                    .with_importance(0.9);
    ...
    fn test_generate_report() {
        ...
        let agent_id = Uuid::new_v4();
        {
            ...
            let memory =
                crate::memory::store::ClientMemory::new(agent_id, 5, "测试记忆".to_string())
                    .with_importance(0.8);
