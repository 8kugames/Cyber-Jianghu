...
impl EventTraitMapper {
    ...
    pub fn apply_to_persona(
        &self,
        event: &WorldEvent,
        persona: &mut DynamicPersona,
        tick_id: i64,
    ) {
        let changes = self.map_event(event, persona, tick_id);

        for change in changes {
            persona.apply_trait_change(
                &change.trait_name,
                change.delta,
                change.reason,
                tick_id,
...
mod tests {
    ...
    fn test_trait_mapping_rules() {
        ...
        // 验证被攻击规则
        let attacked_rule = mapper.rules.iter()
    ...
    fn test_map_event_to_changes() {
        ...
        // 应该有愤怒变化
        let anger_change = changes.iter()
            .find(|c| c.trait_name == "愤怒");
