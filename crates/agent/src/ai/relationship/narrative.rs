...
impl NarrativeGenerator {
    ...
    fn format_persona_traits(&self, persona: &DynamicPersona) -> String {
        // 取最重要的 3 个特质
        let trait_items: Vec<_> = persona.traits.iter()
    ...
    fn extract_description(&self, response: &str) -> Result<String> {
        ...
        let description = description.strip_prefix("描述：")
    ...
    pub async fn update_with_debounce(
        ...
        // 检查是否需要更新（避免重复生成）
        {
            let mut pending = self.pending_generations.lock()
        ...
        match self.generate_description(memory, persona).await {
            Ok(new_description) => {
                if let Err(e) = store.update_self_description(target_id, &new_description, current_tick) {
...
mod tests {
    ...
    fn test_extract_description_with_prefix() {
        ...
        let generator = NarrativeGenerator::new(Arc::new(client));
        let result = generator.extract_description("描述：一位值得信赖的挚友").unwrap();
        assert_eq!(result, "一位值得信赖的挚友");
    }
    ...
    fn test_extract_description_too_long() {
        let long_desc = "这是一个非常非常非常非常非常非常长的描述";
    ...
    fn test_summarize_events_positive() {
        ...
        memory.add_event(crate::ai::relationship::types::KeyEvent::new(1, "help", "帮助", 10));
        memory.add_event(crate::ai::relationship::types::KeyEvent::new(2, "gift", "礼物", 5));
        memory.add_event(crate::ai::relationship::types::KeyEvent::new(3, "help", "帮助", 5));
        let summary = generator.summarize_events(&memory);
    ...
    fn test_summarize_events_negative() {
        ...
        let generator = NarrativeGenerator::new(Arc::new(client));
        let mut memory = create_test_memory(50);
        memory.add_event(crate::ai::relationship::types::KeyEvent::new(1, "attack", "攻击", -10));
        memory.add_event(crate::ai::relationship::types::KeyEvent::new(2, "insult", "侮辱", -5));
        memory.add_event(crate::ai::relationship::types::KeyEvent::new(3, "attack", "攻击", -5));
        let summary = generator.summarize_events(&memory);
    ...
    fn test_summarize_events_mixed() {
        ...
        let mut memory = create_test_memory(50);
        memory.add_event(crate::ai::relationship::types::KeyEvent::new(1, "help", "帮助", 10));
        memory.add_event(crate::ai::relationship::types::KeyEvent::new(2, "attack", "攻击", -10));
        let summary = generator.summarize_events(&memory);
    ...
    async fn test_generate_description_with_llm() {
        ...
        let result = generator.generate_description(&memory, &persona).await.unwrap();
        assert_eq!(result, "一位值得信赖的挚友");
    }
