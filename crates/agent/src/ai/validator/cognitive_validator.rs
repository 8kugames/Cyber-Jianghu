    ...
    fn check_content_length(&self, chain: &CognitiveChain) -> Option<CognitiveValidationResult> {
        for stage_output in &chain.stages {
            ...
            if content_len < self.min_thought_length {
                return Some(CognitiveValidationResult::rejected(
                    format!("{} 阶段内容过短 ({} 字符)", stage_output.stage.name(), content_len),
                    format!(
                        "{} 阶段至少需要 {} 字符的思考内容",
    ...
    fn check_coherence(&self, chain: &CognitiveChain) -> Option<CognitiveValidationResult> {
        ...
        if !has_reasoning {
            ...
            let references_previous = motivation
                .as_ref()
                .map(|m| {
                    decision_lower.contains(&m.content.to_lowercase().chars().take(5).collect::<String>())
                        || decision_lower.contains(&m.content.to_lowercase().chars().take(10).collect::<String>())
                })
                .unwrap_or(false)
                || planning
                    .as_ref()
                    .map(|p| {
                        decision_lower.contains(&p.content.to_lowercase().chars().take(5).collect::<String>())
                            || decision_lower.contains(&p.content.to_lowercase().chars().take(10).collect::<String>())
                    })
                    .unwrap_or(false);
...
mod tests {
    use super::*;
    use crate::agent::CognitiveStage;
    use uuid::Uuid;
    ...
    fn create_test_chain() -> crate::core::cognitive::CognitiveChain {
        ...
        chain.add_stage(crate::agent::StageOutput::new(
    ...
    fn test_validator_rejects_short_content() {
        ...
        for stage in CognitiveStage::all() {
            chain.add_stage(crate::agent::StageOutput::new(stage.clone(), "短".to_string()));
        }
    ...
    fn test_validation_result_rejected() {
        let result = CognitiveValidationResult::rejected(
            "测试原因".to_string(),
            "测试建议".to_string(),
        );
