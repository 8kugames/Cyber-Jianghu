...
impl OpenClawConfig {
    ...
    fn load() -> Result<Self> {
        ...
        let content = std::fs::read_to_string(&config_path)
            .with_context(|| format!("Failed to read OpenClaw config from {}", config_path.display()))?;
        serde_json::from_str(&content)
            .with_context(|| format!("Failed to parse OpenClaw config from {}", config_path.display()))
    }
...
impl DirectLlmClientConfig {
    ...
    pub fn load_from_openclaw_config(mut self) -> Result<Self> {
        ...
        let config = OpenClawConfig::load()
            .context("Failed to load OpenClaw configuration. Ensure ~/.openclaw/openclaw.json exists.")?;
...
impl DirectLlmClient {
    ...
    pub fn openclaw() -> Result<Self> {
        Self::new(DirectLlmClientConfig::new(LlmProvider::OpenClaw, None::<String>))
    }
...
mod tests {
    ...
    fn test_provider_from_str() {
        assert_eq!(LlmProvider::from_str("openclaw"), Some(LlmProvider::OpenClaw));
        assert_eq!(LlmProvider::from_str("OpenClaw"), Some(LlmProvider::OpenClaw));
        assert_eq!(LlmProvider::from_str("openai_compatible"), Some(LlmProvider::OpenAICompatible));
        assert_eq!(LlmProvider::from_str("openai-compatible"), Some(LlmProvider::OpenAICompatible));
    ...
    fn test_provider_defaults() {
        ...
        assert_eq!(LlmProvider::Ollama.default_base_url(), Some("http://localhost:11434/v1"));
    ...
    fn test_direct_client_openclaw_with_url() {
        ...
        assert_eq!(client.config.base_url, Some("http://custom:9999".to_string()));
        assert_eq!(client.config.api_key, None);
    }

    #[test]
    fn test_direct_client_openai_compatible() {
        let client = DirectLlmClient::openai_compatible(
            "https://api.example.com/v1",
            "gpt-4",
            "test-key"
        ).unwrap();
        assert_eq!(client.config.provider, LlmProvider::OpenAICompatible);
        assert_eq!(client.config.base_url, Some("https://api.example.com/v1".to_string()));
    ...
    fn test_direct_client_openai_compatible_missing_fields() {
        // 缺少 base_url 和 model
        assert!(DirectLlmClient::new(
            DirectLlmClientConfig::new(LlmProvider::OpenAICompatible, Some("test-key"))
        ).is_err());
    }
    ...
    fn test_direct_client_ollama() {
        ...
        assert_eq!(client.config.base_url, Some("http://custom:11434/v1".to_string()));
    }
