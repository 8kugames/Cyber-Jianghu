...
pub fn motivation_schema(persona: &DynamicPersona) -> JsonSchema {
    ...
    schema.add_property(
        ...
        SchemaProperty::integer(Some("驱动强度（1-10，10 为最强烈）".to_string()), Some(1), Some(10)),
        true,
    );
...
pub fn planning_schema(_persona: &DynamicPersona) -> JsonSchema {
    ...
    schema.add_property(
        "priority".to_string(),
        SchemaProperty::integer(Some("优先级（1-10，10 为最高）".to_string()), Some(1), Some(10)),
        true,
    );
...
pub fn decision_schema(_persona: &DynamicPersona, available_actions: &[String]) -> JsonSchema {
    ...
    schema.add_property(
        "target".to_string(),
        SchemaProperty::string(Some("目标（可选，如：其他 Agent 名称、物品 ID）".to_string())),
        false,
    );
    ...
    schema.add_property(
        ...
        SchemaProperty::number(Some("决策置信度（0.0-1.0）".to_string()), Some(0.0), Some(1.0)),
        false,
    );
...
impl SchemaValidator {
    ...
    pub fn validate(json: &Value, schema: &JsonSchema) -> Result<(), SchemaValidationError> {
        ...
        for required_field in &schema.required {
            if !json.get(required_field).is_some() {
    ...
    /// 从 JSON 字符串解析并验证
    pub fn parse_and_validate(json_str: &str, schema: &JsonSchema) -> Result<Value, SchemaValidationError> {
