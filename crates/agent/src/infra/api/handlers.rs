...
///
/// GET /api/v1/characters/{id}/soul-cycles
///
/// 用于获取其他角色的经历日志（当前角色使用本地 SQLite）
pub(super) async fn get_character_soul_cycles_handler(
    State(state): State<HttpApiState>,
    AxumPath(agent_id): AxumPath<uuid::Uuid>,
    axum::extract::Query(params): axum::extract::Query<std::collections::HashMap<String, String>>,
) -> impl IntoResponse {
    let page: u32 = params.get("page").and_then(|s| s.parse().ok()).unwrap_or(1);
    ...
    let (device_id, auth_token) = match get_device_id(&state).await {
        ...
        Err(e) => {
            return (
                StatusCode::PRECONDITION_FAILED,
                Json(serde_json::json!({"error": format!("设备身份未初始化: {}", e)})),
            )
                .into_response();
        }
    };
    ...
    let server_url = format!(
        "{}/api/dashboard/agent/{}/experiences?device_id={}&auth_token={}&page={}&limit={}",
        server_http_url, agent_id, device_id, auth_token, page, limit
    );
    ...
    match client.get(&server_url).send().await {
        Ok(resp) => {
            if resp.status() == reqwest::StatusCode::UNAUTHORIZED {
                return (
                    ...
                    Json(serde_json::json!({"error": "认证失败，请重新登录"})),
                )
                    .into_response();
            }
            if resp.status() == reqwest::StatusCode::NOT_FOUND {
                return (
                    StatusCode::NOT_FOUND,
                    Json(serde_json::json!({"error": "未找到该角色的经历日志"})),
                )
                    .into_response();
            }
            match resp.json::<serde_json::Value>().await {
                ...
                Err(e) => {
                    tracing::error!("解析 Server 响应失败: {}", e);
                    (
                        StatusCode::BAD_GATEWAY,
                        Json(serde_json::json!({"error": "获取经历日志失败"})),
                    )
                        .into_response()
                }
            }
        ...
        Err(e) => {
            tracing::error!("连接 Server 失败: {}", e);
            (
                StatusCode::BAD_GATEWAY,
                Json(serde_json::json!({"error": "无法连接服务器"})),
            )
                .into_response()
        }
    }
