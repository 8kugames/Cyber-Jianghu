...
/// 从 server dashboard 获取经历日志（proxy 模式，确保与 server-web 显示一致）
///
/// GET /api/v1/character/experiences?page=1&limit=20
pub(super) async fn get_experiences_from_server_handler(
    State(state): State<HttpApiState>,
    ...
    let limit: u32 = params.get("limit").and_then(|s| s.parse().ok()).unwrap_or(20).min(50);
    ...
    let (device_id, auth_token) = match get_device_id(&state).await {
        ...
        Err(e) => {
            ...
            return (
                StatusCode::PRECONDITION_FAILED,
                Json(serde_json::json!({"error": "设备身份未初始化"})),
            )
                .into_response();
        }
    };
    ...
    let server_url = format!(
        "{}/api/dashboard/agent/{}/experiences?page={}&limit={}&device_id={}&auth_token={}",
        server_http_url, agent_id, page, limit, device_id, auth_token
    );
    ...
    let resp = match client.get(&server_url).send().await {
        Ok(r) => r,
        Err(e) => {
            error!("连接 server 失败: {}", e);
            return (StatusCode::BAD_GATEWAY, Json(serde_json::json!({"error": "无法连接到服务器"}))).into_response();
        }
    };
    ...
    if !resp.status().is_success() {
        ...
        return (StatusCode::from_u16(status.as_u16()).unwrap_or(StatusCode::BAD_GATEWAY), Json(serde_json::json!({"error": format!("服务器错误: {}", status.as_u16())}))).into_response();
    }

    #[derive(Debug, Deserialize)]
    struct ServerExperiencesResponse {
        experiences: Vec<ServerExperienceEntry>,
        total: i64,
        page: i32,
        limit: i32,
    }
    ...
    let server_resp: ServerExperiencesResponse = match resp.json().await {
        Ok(r) => r,
        Err(e) => {
            error!("解析 server 响应失败: {}", e);
            return (StatusCode::BAD_GATEWAY, Json(serde_json::json!({"error": "服务器响应格式错误"}))).into_response();
        }
    };

    #[derive(Debug, Serialize)]
    struct TransformedAttemptEntry {
        tick_id: i64,
        world_time: Option<serde_json::Value>,
        created_at: String,
        attempt: i32,
        renhun: TransformedRenhun,
        tianhun: TransformedTianhun,
        dihun: TransformedDihun,
        final_intent: Option<TransformedFinalIntent>,
    }

    #[derive(Debug, Serialize)]
    struct TransformedRenhun {
        narrative: Option<String>,
        thought_log: Option<String>,
    }

    #[derive(Debug, Serialize)]
    struct TransformedTianhun {
        action_type: Option<String>,
        action_data: Option<serde_json::Value>,
        speech_content: Option<String>,
        success: bool,
    ...
    #[derive(Debug, Serialize)]
    struct TransformedDihun {
        result: Option<String>,
        layers: Vec<TransformedLayer>,
        reason: Option<String>,
        narrative: Option<String>,
    }
    ...
    #[derive(Debug, Serialize)]
    struct TransformedImmediateEntry {
        intent_id: String,
        route_type: String,
        action_type: String,
        action_data: Option<serde_json::Value>,
    ...
    let mut records_map: HashMap<String, Vec<serde_json::Value>> = HashMap::new();
    let mut immediate_map: HashMap<String, Vec<serde_json::Value>> = HashMap::new();

    for exp in server_resp.experiences {
        let tick_key = exp.tick_id.to_string();
        let created_at_str = exp.created_at.to_rfc3339();

        if let Some(metadata) = exp.soul_cycle_metadata {
            if let Ok(meta) = serde_json::from_value::<serde_json::Value>(metadata) {
                let world_time = meta.get("world_time").cloned();

                if let Some(cycles) = meta.get("cycles").and_then(|c| c.as_array()) {
                    for (idx, cycle) in cycles.iter().enumerate() {
                        let attempt_entry = TransformedAttemptEntry {
                            tick_id: exp.tick_id,
                            world_time: world_time.clone(),
                            created_at: created_at_str.clone(),
                            attempt: cycle.get("attempt").and_then(|a| a.as_i64()).unwrap_or((idx + 1) as i64) as i32,
                            renhun: TransformedRenhun {
                                narrative: cycle.get("renhun").and_then(|r| r.get("narrative")).and_then(|n| n.as_str()).map(String::from),
                                thought_log: cycle.get("renhun").and_then(|r| r.get("thought_log")).and_then(|t| t.as_str()).map(String::from),
                            },
                            tianhun: TransformedTianhun {
                                action_type: cycle.get("tianhun").and_then(|t| t.get("action_type")).and_then(|a| a.as_str()).map(String::from),
                                action_data: cycle.get("tianhun").and_then(|t| t.get("action_data")).cloned(),
                                speech_content: cycle.get("tianhun").and_then(|t| t.get("speech_content")).and_then(|s| s.as_str()).map(String::from),
                                success: cycle.get("tianhun").and_then(|t| t.get("success")).and_then(|s| s.as_bool()).unwrap_or(true),
                                error: cycle.get("tianhun").and_then(|t| t.get("error")).and_then(|e| e.as_str()).map(String::from),
                            },
                            dihun: TransformedDihun {
                                result: cycle.get("dihun").and_then(|d| d.get("result")).and_then(|r| r.as_str()).map(String::from),
                                layers: cycle.get("dihun").and_then(|d| d.get("layers")).and_then(|l| l.as_array()).map(|arr| {
                                    arr.iter().filter_map(|l| {
                                        Some(TransformedLayer {
                                            layer: l.get("layer")?.as_str()?.to_string(),
                                            passed: l.get("passed")?.as_bool()?,
                                            detail: l.get("detail").and_then(|d| d.as_str()).map(String::from),
                                        })
                                    }).collect()
                                }).unwrap_or_default(),
                                reason: cycle.get("dihun").and_then(|d| d.get("reason")).and_then(|r| r.as_str()).map(String::from),
                                narrative: cycle.get("dihun").and_then(|d| d.get("narrative")).and_then(|n| n.as_str()).map(String::from),
                            },
                            final_intent: cycle.get("final_intent").and_then(|fi| fi.as_object()).map(|obj| TransformedFinalIntent {
                                intent_id: obj.get("intent_id").and_then(|i| i.as_str()).map(String::from),
                                action_type: obj.get("action_type").and_then(|a| a.as_str()).map(String::from),
                                action_data: obj.get("action_data").cloned(),
                            }),
                        };
                        records_map.entry(tick_key.clone()).or_default().push(serde_json::to_value(attempt_entry).unwrap_or_default());
                    }
                }

                if let Some(immediate) = meta.get("immediate_intents").and_then(|i| i.as_array()) {
                    for imm in immediate {
                        let entry = TransformedImmediateEntry {
                            intent_id: imm.get("intent_id").and_then(|i| i.as_str()).unwrap_or("").to_string(),
                            route_type: imm.get("route_type").and_then(|r| r.as_str()).unwrap_or("").to_string(),
                            action_type: imm.get("action_type").and_then(|a| a.as_str()).unwrap_or("").to_string(),
                            action_data: imm.get("action_data").cloned(),
                            speech_content: imm.get("speech_content").and_then(|s| s.as_str()).map(String::from),
                            send_status: imm.get("send_status").and_then(|s| s.as_str()).unwrap_or("").to_string(),
                            send_error: imm.get("send_error").and_then(|e| e.as_str()).map(String::from),
                        };
                        immediate_map.entry(tick_key.clone()).or_default().push(serde_json::to_value(entry).unwrap_or_default());
                    }
                }
            }
        }
    ...
    let has_more = (page as i64 * limit as i64) < server_resp.total;

    Json(serde_json::json!({
        "page": page,
        "limit": limit,
        "total": server_resp.total,
        "has_more": has_more,
        "records": records_map,
