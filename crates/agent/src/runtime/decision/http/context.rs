...
use crate::ai::cognitive::narrative::{NarrativeEngine, NarrativeConfig};
...
pub fn create_attributes_glimpse(
    ...
    // 定义属性类别（基于名称前缀或具体名称）
    let status_attrs = ["hp", "stamina", "hunger", "thirst", "qi", "sanity", "reputation"];
    let primary_attrs = ["strength", "agility", "constitution", "intelligence", "charisma", "luck"];
    // 派生属性当前不在 WorldState 中，这里列出已知名称
    let derived_attrs = [
        "max_carry_weight", "physical_damage", "dodge_rate", "critical_rate",
        "skill_learning_speed", "exp_bonus", "stamina_regen", "qi_regen",
        "perception", "social_interaction", "event_probability",
    ];
...
fn generate_impl(
    ...
    sections.push(format!("- **{}** ({})", state.location.name, state.location.node_type));

    // 自身状态 - 叙事化描述（不暴露数值）
    sections.push("".to_string());
    sections.push("## 自身状态".to_string());

    let attrs: HashMap<String, i32> = state.self_state.attributes.iter()
        .map(|(k, v)| (k.clone(), *v))
        .collect();
    ...
    for (name, value) in &attrs {
        if !standard.contains(&name.as_str()) {
            sections.push(format!("- {}: {}", name, engine.describe_attribute(name, *value)));
        }
    }
    ...
    if !narrative.status_effects.is_empty() {
        sections.push("".to_string());
        sections.push(format!("**特殊状态**: {}", narrative.status_effects.join("、")));
    }
    ...
    if !state.entities.is_empty() {
        for entity in &state.entities {
            let dist = if entity.distance > 0 { format!(" ({}m)", entity.distance) } else { String::new() };

            let rel = match relationship_store {
                Some(store) => store.get_relationship(entity.id).ok().flatten()
                    .map(|mem| {
    ...
    if !state.available_actions.is_empty() {
        ...
        let actions: Vec<String> = state.available_actions.iter()
...
pub fn create_narrative_engine() -> NarrativeEngine {
    ...
    if let Some(home) = dirs::home_dir() {
        let config_path = home.join(".cyber-jianghu").join("config").join("narrative_config.json");

        if config_path.exists() {
            match NarrativeConfig::from_file(&config_path) {
                Ok(config) => {
                    tracing::info!("[context] Loaded narrative config from {}", config_path.display());
