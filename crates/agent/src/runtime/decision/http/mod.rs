...
use crate::ai::relationship::{RelationshipStore, NarrativeGenerator};
...
use crate::ai::validator::{Validator, RuleEngineValidator};
use crate::ai::cognitive::narrative::NarrativeEngine;

// 重导出 context 模块的公共 API
pub use context::{ContextResponse, AttributesGlimpse, create_attributes_glimpse, create_narrative_engine};
...
pub async fn run_http_server(port: u16, api_state: HttpApiState) -> anyhow::Result<()> {
    let app = Router::new()
        ...
        // === 关系管理端点 ===
        .route("/api/v1/relationship/list", get(handlers::get_relationships_handler))   // 获取所有关系
        .route("/api/v1/relationship/{id}", get(handlers::get_relationship_handler))    // 获取特定关系
        .route("/api/v1/relationship", post(handlers::update_relationship_handler))     // 更新关系

        // === 寿命端点 ===
        .route("/api/v1/lifespan", get(handlers::get_lifespan_handler))  // 获取寿命状态

        // === 记忆管理端点 ===
        .route("/api/v1/memory/recent", get(handlers::get_recent_memory_handler))  // 获取近期记忆
        .route("/api/v1/memory/search", post(handlers::search_memory_handler))     // 搜索记忆 TODO: 语义搜索待实现
...
pub fn create_http_state(agent_id: Uuid) -> (Arc<HttpDecisionState>, HttpApiState) {
    ...
    // 初始化关系存储
    let relationship_store = RelationshipStore::open(agent_id, &data_dir.join("relationships.db"))
        .ok()
        .map(Arc::new);
    ...
    // 初始化寿命计算器（使用默认配置）
    let lifespan_calculator = Some(Arc::new(Mutex::new(LifespanCalculator::with_default_config())));

    // 初始化对话客户端（使用空操作处理器）
    // 实际对话事件处理由外部系统通过 API 完成
    let dialogue_handler = Arc::new(NoopDialogueHandler::default());
    let dialogue_client = Some(Arc::new(DialogueClient::new(agent_id, dialogue_handler)));

    // 初始化意图验证器（使用默认规则引擎验证器）
    let intent_validator = Some(Arc::new(RuleEngineValidator::with_default_config()) as Arc<dyn Validator>);
    ...
    pub async fn maybe_update_narratives(&self, world_state: &WorldState) {
        ...
        for entity in &world_state.entities {
            ...
            tokio::spawn(async move {
                let _ = generator.update_with_debounce(
                    target_id,
                    current_tick,
                    &memory,
                    &persona_clone,
