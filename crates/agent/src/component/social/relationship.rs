...
// 关系名册同步（sync_roster / is_generic_name）及其测试位于兄弟模块 roster_sync.rs：
// relationship.rs 越过项目 800 行上限约束后的机械迁移（triple-review F1）。
...
mod tests {
    ...
    fn test_record_social_event_new_target() {
        ...
        // 记录社交事件（新目标）
        store
            .record_social_event(target_id, "李四", 10, "予", "李四给了你一个馒头", 5)
            .unwrap();
        ...
        assert_eq!(rel.key_events[0].event_type, "予");
    }
    ...
    fn test_record_social_event_accumulates_favorability() {
        ...
        // 多次事件
        store
            .record_social_event(target_id, "王五", 5, "予", "送了馒头", 5)
            .unwrap();
        store
            .record_social_event(target_id, "王五", 10, "交易", "公平交易", 3)
            .unwrap();
        store
            .record_social_event(target_id, "王五", 15, "取", "取了银子", -15)
            .unwrap();
    ...
    fn test_record_social_event_favorability_clamped() {
        ...
        // 超过上限
        store
            .record_social_event(target_id, "赵六", 1, "予", "大礼", 80)
            .unwrap();
        store
            .record_social_event(target_id, "赵六", 2, "予", "再送礼", 50)
            .unwrap();
