...
use crate::component::social::{RelationshipStore, relationship_narrative::NarrativeGenerator};
use anyhow::Result;
...
pub async fn migrate_relationship_descriptions(
    ...
    for memory in all_relationships {
        ...
        match generator.generate_description(&memory, persona).await {
            Ok(description) => {
                if let Err(e) =
                    store.update_self_description(memory.target_agent_id, &description, 0)
                {
...
mod tests {
    ...
    async fn test_migration_empty_database() {
        ...
        let report = migrate_relationship_descriptions(&store, &generator, &persona)
            .await
            .unwrap();
    ...
    async fn test_migration_with_existing_descriptions() {
        ...
        // 添加一个已有描述的关系
        let mut memory = crate::component::social::relationship_types::RelationshipMemory::new(
            Uuid::new_v4(),
            "已有描述的目标",
        );
    ...
    async fn test_migration_generates_descriptions() {
        ...
        // 添加一个没有描述的关系
        let memory = crate::component::social::relationship_types::RelationshipMemory::new(
            Uuid::new_v4(),
            "需要迁移的目标",
        );
        store.upsert_relationship(&memory).unwrap();
        ...
        // 验证描述已保存
        let retrieved = store
            .get_relationship(memory.target_agent_id)
            .unwrap()
            .unwrap();
