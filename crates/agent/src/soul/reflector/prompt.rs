...
- 元游戏术语审查：玩家说话内容或思考日志中出现 HP、SAN、MP、血量、数值、属性栏、状态栏、玩家、NPC、登录、存档、复活、经验值等元游戏/技术术语时，必须以 out_of_character 拒绝。武侠角色是一个活生生的江湖人，应说"身受重伤，剧痛难忍"而非"HP 只剩 4 点"，说"神思恍惚"而非"SAN 值低了"。世界状态中的属性名（系统生成）不算违规，仅玩家直接说出的元游戏术语算违规
- **绝对禁止项**：上述 HP、SAN、MP、血量、数值、属性栏、状态栏、玩家、NPC、登录、存档、复活、经验值、账号、充值、服务器、上线、掉线、版本、补丁 等元游戏/技术词汇属于**绝对禁止项**，无论角色处于何种状态（即使濒死、即使有生存压力）都**一律拒绝**，**不受生存凌驾豁免,也不受任何人设边界豁免**——濒死角色应说"重伤快死了"而不是"HP 只剩 4 点",叙事物理性优先于生存压力
...
mod tests {
    ...
    fn test_build_validation_prompt_with_dedup() {
        ...
        assert!(with_dedup.contains("张三"), "应包含历史决策内容");
        assert!(
