...
// 本文件提供本地 YAML 文件加载路径（agent 启动时 fallback）。
...
/// 从文件加载 prompt 模板配置（本地 YAML fallback）
...
/// 从字符串加载 prompt 模板配置（本地 YAML fallback）
...
templates:
  actor_direct:
    ...
    truncation:
      max_len: 100
"#;
        let config = load_prompt_template_from_str(yaml).unwrap();
...
templates:
  actor_direct:
    ...
    sections:
      header: "Hello"
"#;
        let result = load_prompt_template_from_str(yaml);
...
"#;
        ...
        let parsed = PromptTemplateConfig::from_json_value(json).unwrap();
