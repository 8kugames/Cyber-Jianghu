...
- **Game Data System**: Loads JSON configs from `crates/server/config/*.json`
...
All game mechanics are configured via JSON files in `crates/server/config/`:
- `actions.json` - Action definitions with parameters and validation rules
- `attributes.json` - Attribute definitions (primary, status, derived)
- `items.json` - Item definitions with properties
- `locations.json` - Location graph with nodes and edges
- `narrative_config.json` - Threshold-based attribute descriptions
- `recipes.json` - Crafting recipes
- `game_rules.json` - Core game rules
...
2. **Data-driven**: Configure via `crates/server/config/*.json`, not hardcoded values
...
| Server configuration | `crates/server/config/*.json` |
| Database migrations | `crates/server/migrations/*.sql` |
| Docker stack | `docker-compose.yml`, `docker-compose.prod.yml` |
| Agent templates | `integration/openclaw/templates/*.json5` |
| OpenClaw hooks | `integration/openclaw/hooks/*.ts` |
...
1. Server loads `narrative_config.json` on startup via `GameDataLoader`
2. Agent registration response includes `narrative_config` field
3. Agent stores config to `~/.cyber-jianghu/config/narrative_config.json`
...
- All game mechanics are configurable via JSON files
