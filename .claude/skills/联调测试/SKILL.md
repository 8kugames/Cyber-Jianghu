...
每端口强依赖：generate→register→verify 必须串行；端口间独立，用 `xargs -P 6` 并发。
...
1. **BuildKit 缓存**：`--no-cache` 不能清除 BuildKit cache mount。如需强制重编译，修改 Dockerfile 中 cache id 或使用 `--build-arg CACHEBUST=$(date +%s)`
2. **Tick 时长**：60s（`game_rules.yaml` 的 `real_seconds_per_tick`），所有 per-tick 计算基于此
3. **Token 持久化**：`$CYBER_JIANGHU_DATA_DIR/logs/token_cost_count.tmp`，每次 tick 由 `persist_and_reset()` 写入
4. **Agent 端口映射**：agent-1=23341, agent-2=23342, ..., agent-5=23345, agent-ollama=23349
5. **容器命名**：test-agent-1 ~ test-agent-5, test-agent-ollama
6. **报告路径**：`logs/测试报告/联调测试.{MMDD}.docker.{N}.md`
7. **配置冻结**：测试开始后不得修改任何 agent 或 server 配置。如需调整，记录变更点并重新开始
