...
collect_round() {
  ...
  {
    echo "[server_health]"
    curl -sf --max-time 5 http://HOST_REDACTED:23333/health || echo "FAIL"
  } >> "$round_log"
  ...
  for entry in "${AGENTS[@]}"; do
    ...
    mkdir -p "$cdir"
    local token
    token=$(docker exec "$c" grep '^auth_token:' /app/data/servers/HOST-REDACTED-DASHED-23333/device.yaml 2>/dev/null | awk '{print $2}')
    if [ -z "$token" ]; then
