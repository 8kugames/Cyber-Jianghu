...
| **三级记忆** | 工作记忆 (自动注入) / 情景记忆 (自动注入) / 语义记忆 (地魂工具按需检索)，支撑人格连贯性和行为一致性 |

**决策机制**（由三魂架构实现）

| 特性 | 说明 |
|------|------|
| **多意图管道** | 单 tick 可提交多个 Intent，顺序执行；后续步骤失败会中断批次，但不会回滚已成功步骤 |
...
| **内置管理面板** | 角色创建、状态查看、托梦注入、YAML 配置热更新 |
